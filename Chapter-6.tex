...
In this chapter, an autoencoder-based SIS framework is considered, where the encoder transforms the input image into a compressed latent space. This latent representation is then divided into multiple independent shares. These shares are distributed such that only a sufficient combination of them enables reconstruction. The decoder utilizes the combined latent information to recover the original image.
...
Nag et al.~\cite{nag_boolean} proposed a Boolean-based MSIS scheme where share size can increase depending on the number of secrets, leading to higher storage requirements. Lin et al.~\cite{lin2024} developed a key-free SIS scheme using a Cycling-XOR mechanism, embedding shares into cover images for improved concealment. However, the share size is still not reduced.

To address storage inefficiency, Ou et al.~\cite{ou2015userfriendly} proposed a scheme based on block truncation coding and error diffusion, achieving significant reduction in share size. Similarly, Purushothama B. R.~\cite{purushothama_size} introduced a method that reduces shadow image dimensions to $\lceil h/n \rceil \times w$, improving storage efficiency compared to traditional XOR schemes.

Further advancements include extending XOR-based methods to multi-secret scenarios. Kabirirad and Eslami~\cite{kabirirad2018t} proposed a $(t,n)$ threshold MSIS scheme using Boolean operations, enabling flexible reconstruction. However, the method depends on sequential share availability and does not reduce share size. Faraoun~\cite{faraoun2020} proposed a secure MSIS scheme combining stream ciphers and XOR-based key sharing, but the generated shares remain equal in size to the original images.
...
The proposed method achieves high reconstruction quality with strong structural similarity, ensures security by preventing information leakage from individual shares, and significantly reduces storage requirements. However, the reconstruction is not perfectly lossless due to compression in the latent space, which introduces minor smoothing artifacts in the reconstructed images. Figure~\ref{fig:results_autoencoder} shows the visual reconstruction results alongside the generated shares.
...
The model achieves high reconstruction quality ($\mathrm{PSNR} > 30\,\mathrm{dB}$, $\mathrm{SSIM} \approx 1.0$), while individual shares remain dissimilar to the original, ensuring strong confidentiality. It also reduces storage by up to $98\%$.
