...
\label{sec:lad-work}
% ─────────────────────────────────────────────────────────────────────────────
The basic problem in SIS is to strike a balance between security, reconstruction accuracy, and storage space efficiency. The existing approaches tend to focus on security with the penalty of storage space overhead. The contributions of this chapter are as follows:
...
Moreover, Figure~\ref{fig:cameraman_reconstruction_comparison} illustrates the reconstruction attempts with only three out of the four shares and two out of the four shares on the Cameraman test image. In this comparison, Figure~\ref{fig:cameraman_orig2} shows the original image, Figures~\ref{fig:cameraman_2share} and~\ref{fig:cameraman_3share} show the partial reconstructions, and Figures~\ref{fig:cameraman_shadow1}, \ref{fig:cameraman_shadow2}, \ref{fig:cameraman_shadow3}, and~\ref{fig:cameraman_shadow4} show the corresponding shadow images. The reconstructed images are obviously noisy and do not have any resemblance to the original hidden image. This verifies that our scheme satisfies the basic requirement of SIS, which is that no information is revealed with less than the threshold number of shares.
