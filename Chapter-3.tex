...
trained for 20 epochs using the Adam optimizer~\cite{kingma2015adam}. The trained
...
residual connections~\cite{he2016deep}. The CNN includes an initial
...
batch normalization~\cite{ioffe2015batch}, dropout~\cite{srivastava2014dropout},
...
The reconstructions are highly similar to the input, with global structure and fine details being preserved. The high values for SSIM and PSNR suggest that the neural network successfully reconstructs an input without loss, which is very similar to classical XOR decoding. This finding supports previous research, which shows that residual CNNs can decode structured linear transformations if enough input is present~\cite{he2016deep}.
...
bounds of the XOR protocol~\cite{tuyls2005capacity} and other works involving
neural network decoding with missing input data~\cite{pathak2016context}.
